...
export let attributeMedia = new Schema({
...
let AttributeSchema = new Schema({
  ...
  displayName: { type: String, trim: true, required: true, index: false },
...
AttributeSchema.index({ "searchKeywords": 1, "name": 1 });

export let attributes: Model<IAttribute> = mongoose.model<IAttribute>(config.DB_MODEL_REF.ATTRIBUTE, AttributeSchema);