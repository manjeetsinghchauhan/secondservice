...
import { validateCreateServiceLocation, validateUpdateServiceLocation } from "./routeValidater";
...
export const serviceLocationRoutes = [
    {
        ...
        handler: async (request: any, h: ResponseToolkit) => {
            try {
                ...
                const result = await serviceLocationController.createServiceLocation(payload);
        ...
        options: {
            ...
            validate: {
                ...
                payload: validateCreateServiceLocation,
    ...
    {
        method: "PUT",
        path: `${SERVER.API_BASE_URL}/admin/service-locations/{serviceLocationId}`,
        handler: async (request: any, h: ResponseToolkit) => {
            try {
                const payload = { ...request.params, ...request.payload };
                const tokenData: TokenData = request.auth?.credentials?.tokenData;
                const result = await serviceLocationController.updateServiceLocation(payload);
                return responseHandler.sendSuccess(request, h, result);
            } catch (error) {
                return responseHandler.sendError(request, error);
            }
        ...
        options: {
            ...
            validate: {
                headers: authorizationHeaderObj,
                params: Joi.object({
                    serviceLocationId: Joi.string().trim().regex(/^[a-f\d]{24}$/i).required().description("Service Location ID")
                }),
                payload: validateUpdateServiceLocation,
                failAction: failActionFunction,
                options: {
                    abortEarly: false
                }
